...
int main(){
  ...
  for (int i = 0; i < 2; i++) {
    ...
    printf("\n--- RETRANSMISSAO DE PACOTES COM ERRO ---\n");

if (!estaVaziaPilha(pilhaErro)) {

    Pacote retransmitido = desempilhar(pilhaErro);

    printf("Pacote %d retransmitido com sucesso!\n",
           retransmitido.numeroPacote);

} else {

    printf("Nenhum pacote na pilha de erros.\n");

}
    printf("\nBuscando pacote 99...\n");
    buscarPacote(pacotesAtivos, 99);
